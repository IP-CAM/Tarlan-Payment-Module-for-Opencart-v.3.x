...
class Tarlanpayments
{
    ...
    public function __construct($registry)
    {
        spl_autoload_register(function ($class) {
            if (strpos($class, 'TarlanPayments\\') === 0) {
                $class_file = str_replace('\\', DIRECTORY_SEPARATOR, substr($class, 14)) . '.php';
        ...
        $this->gateway = new \TarlanPayments\Gateway\Gateway($this->config->get('payment_tarlanpayments_gateway_uri'));
        $this->gateway->auth()
            ->setAccountId((int) $this->config->get('payment_tarlanpayments_account_id'))
            ->setSecretKey((string) $this->config->get('payment_tarlanpayments_secret_key'));
    }
