...
class ModelExtensionPaymentTarlanpayments extends Model
{
    ...
    {
        return $this->db->query("SELECT * FROM `" . DB_PREFIX . "tarlanpayments_transaction` WHERE transaction_id='" . (int) $transaction_id . "'")->row;
    }
    ...
    {
        $sql = "SELECT COUNT(*) as total FROM `" . DB_PREFIX . "tarlanpayments_transaction` WHERE 1";
    ...
    {
        $this->db->query("UPDATE `" . DB_PREFIX . "tarlanpayments_transaction` SET is_refunded='1', refunded_at=NOW(), refunds='" . $this->db->escape(json_encode($refunds)) . "' WHERE transaction_id='" . (int) $transaction_id . "'");
    }
    ...
    public function getOrderStatusId($order_id, $transaction_status = null)
    {
        if ($transaction_status) {
            ...
            return $this->config->get('payment_tarlanpayments_transaction_status_' . strtolower($status_name));
        } else {
    ...
    {
        $this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "tarlanpayments_transaction` (
    ...
    {
        $this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "tarlanpayments_transaction`");
    }
}
