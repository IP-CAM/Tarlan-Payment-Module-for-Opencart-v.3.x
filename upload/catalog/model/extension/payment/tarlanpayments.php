...
class ModelExtensionPaymentTarlanpayments extends Model
{
    ...
    {
        $geo_zone_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int) $this->config->get('payment_tarlanpayments_geo_zone_id') . "' AND country_id = '" . (int) $address['country_id'] . "' AND (zone_id = '" . (int) $address['zone_id'] . "' OR zone_id = '0')");

        $tarlanpayments_display_name = $this->config->get('payment_tarlanpayments_display_name');

        $this->load->language('extension/payment/tarlanpayments');

        if (! empty($tarlanpayments_display_name[$this->config->get('config_language_id')])) {
            $title = $tarlanpayments_display_name[$this->config->get('config_language_id')];
        } else {
            $title = $this->language->get('text_default_tarlanpayments_name');
        }

        $status = true;

        $minimum_total = (float) $this->config->get('payment_tarlanpayments_total');

        $tarlanpayments_geo_zone_id = $this->config->get('payment_tarlanpayments_geo_zone_id');

        if ($minimum_total > 0 && $minimum_total > $total) {
            $status = false;
        } else if (empty($tarlanpayments_geo_zone_id)) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                ...
                'title' => $title,
                'terms' => '',
                'sort_order' => (int) $this->config->get('payment_tarlanpayments_sort_order')
            );
        }
    ...
    public function isCaptureOnGatewaySide()
    {
        ...
        return Tarlanpayments::CAPTURE_GW == $this->config->get('payment_tarlanpayments_capture_method');
    }
    ...
    public function addTransaction($transaction_id, $transaction_status, $payment_method, $amount, $currency, $order_id, $user_ip)
    {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "tarlanpayments_transaction` SET transaction_guid='" . $this->db->escape($transaction_id) . "', order_id='" . (int) $order_id . "', transaction_status='" . (int) $transaction_status . "', payment_method='" . (int) $payment_method . "', transaction_amount='" . (float) $amount . "', transaction_currency='" . $this->db->escape($currency) . "', device_ip='" . $this->db->escape($user_ip) . "', created_at=NOW(), is_refunded='0', refunded_at='', refunds='" . $this->db->escape(json_encode(array())) . "'");
    }

    public function getTransactions($filters = array())
    {
        $sql = "SELECT * FROM `" . DB_PREFIX . "tarlanpayments_transaction` WHERE 1";
    ...
    {
        $this->db->query("UPDATE `" . DB_PREFIX . "tarlanpayments_transaction` SET transaction_status='" . (int) $transaction_status . "' WHERE transaction_id='" . (int) $transaction_id . "'");
    }

    public function recurringPayments()
    {
        return (bool) $this->config->get('payment_tarlanpayments_recurring_status');
    }

    public function validateCRON()
    {
        if (! $this->config->get('payment_tarlanpayments_status') || ! $this->config->get('payment_tarlanpayments_recurring_status')) {
            return false;
        }

        if (isset($this->request->get['cron_token']) && $this->request->get['cron_token'] == $this->config->get('payment_tarlanpayments_cron_token')) {
            return true;
        }

        if (defined('TARLANPAYMENTS_ROUTE')) {
            return true;
        }
    ...
    public function nextRecurringPayments()
    {
        ...
        $recurring_sql = "SELECT * FROM `" . DB_PREFIX . "order_recurring` `or` INNER JOIN `" . DB_PREFIX . "tarlanpayments_transaction` st ON (st.transaction_guid = `or`.reference) WHERE `or`.status='" . self::RECURRING_ACTIVE . "'";
        ...
        foreach ($this->db->query($recurring_sql)->rows as $recurring) {
            ...
            $payments[] = array(
                ...
                'amount' => $this->tarlanpayments->lowestDenomination($price * $recurring['product_quantity'], $recurring['transaction_currency']),
    ...
    private function getLastSuccessfulRecurringPaymentDate($order_recurring_id)
    {
        ...
        return $this->db->query("SELECT date_added FROM `" . DB_PREFIX . "order_recurring_transaction` WHERE order_recurring_id='" . (int) $order_recurring_id . "' AND type IN ('" . Tarlanpayments::STATUS_SUCCESS . "', '" . Tarlanpayments::STATUS_HOLD_OK . "') ORDER BY date_added DESC LIMIT 0,1")->row['date_added'];
    }
    ...
    private function getTotalSuccessfulPayments($order_recurring_id)
    {
        ...
        return $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "order_recurring_transaction` WHERE order_recurring_id='" . (int) $order_recurring_id . "' AND type IN ('" . Tarlanpayments::STATUS_SUCCESS . "', '" . Tarlanpayments::STATUS_HOLD_OK . "')")->row['total'];
    }
    ...
    public function cronEmail($result)
    {
        ...
        $mail->setTo($this->config->get('payment_tarlanpayments_cron_email'));
